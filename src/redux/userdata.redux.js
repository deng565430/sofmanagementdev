...
function updataClientInfo(client) {
  ...
  let checkedScore = ''
  client.score.filter(v => {
    if (v.value) {
      checkedScore = v.value
    }
    return v
  })
  client.checkedScore = checkedScore
      ...
      if (res.data.draw === 0) {
        ...
        if (res.data.data.length > 0) {
          ...
          for (let i = 0; i < datalist.length; i++) {
            const msg = datalist[i].msg ? (datalist[i].msg.length > 10 ? datalist[i].msg.substr(0, 8) + '...' : datalist[i].msg) : null
            const actionMsg = datalist[i].msg ? datalist[i].msg : null
